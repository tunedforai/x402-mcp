...
async function parseResponse(
  res: Response,
): Promise<{ result?: { content?: unknown }; error?: { message?: string } }> {
...
server.tool(
  "marketAnalyze",
  "Macro regime plus directional signal and confidence for a token.",
...
server.tool(
  "marketFull",
  "Full market data bundle: snapshot, orderflow, and LLM-generated verdict.",
  { token: z.string().default("BTC").describe("Token symbol") },
  async ({ token }) => {
...
server.tool(
  ...
  { address: z.string().describe("Full 42-character Ethereum address or supported Solana address") },
  async ({ address }) => {
